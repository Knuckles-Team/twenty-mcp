...
def register_graphql_tools(mcp: FastMCP):
    ...
    async def twenty_discover_graphql_schema(
        type_name: str | None = Field(
            default=None,
            description="Optional specific GraphQL type name to inspect details for (e.g. 'Person', 'Company'). If omitted, lists all available types in the schema.",
        ),
        client=Depends(get_graphql_client),
        ctx: Context | None = Field(
            default=None, description="MCP context for progress reporting"
        ...
        """Discover the dynamic Twenty GraphQL schema in real-time.

        Note: Twenty disables GraphQL introspection, so this will typically
        return an ``{"error": ...}`` payload rather than schema details.
        """
        ...
        if ctx:
            await ctx.info("Retrieving dynamic Twenty GraphQL schema...")
        ...
        try:
            ...
            return {"error": f"Failed to discover Twenty GraphQL schema: {str(e)}"}
